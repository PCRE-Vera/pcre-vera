...
func TestCompileAndMatch(t *testing.T) {
	...
	ovector, err := re.Match([]byte("mail: alice@example"), 0, 0, DefaultLimits())
	if err != nil {
		t.Fatal(err)
	}
...
func TestNoMatchIsNeitherAResultNorAnError(t *testing.T) {
	...
	ovector, err := re.Match([]byte("xyz"), 0, 0, DefaultLimits())
...
func TestAGroupThatDidNotParticipateIsMinusOne(t *testing.T) {
	...
	ovector, err := re.Match([]byte("b"), 0, 0, DefaultLimits())
	if err != nil {
		t.Fatal(err)
	}
...
func TestUnknownOptionBitsAreRefused(t *testing.T) {
	...
	if _, err := re.Match([]byte("a"), 0, 1<<20, DefaultLimits()); err == nil {
...
func TestAStartOffsetPastTheSubjectIsBadInput(t *testing.T) {
	re := MustCompile("a", Options{})
	_, err := re.Match([]byte("aaa"), 4, 0, DefaultLimits())
	var failure *Error
	if !errors.As(err, &failure) || failure.Kind != BadInput {
		t.Fatalf("Match = %v, want BadInput", err)
	}
	if _, err := re.Match([]byte("aaa"), 3, 0, DefaultLimits()); err != nil {
...
func TestALimitPastWhatAnyTargetCouldHonorIsBadInput(t *testing.T) {
	...
	var failure *Error
	if _, err := re.Match([]byte("a"), 0, 0, limits); !errors.As(err, &failure) ||
...
func TestAMatchOverBudgetIsResourceExceeded(t *testing.T) {
	...
	_, err := re.Match([]byte(strings.Repeat("a", 40)), 0, 0, limits)
	var failure *Error
...
func TestACompiledPatternSurvivesConcurrentUse(t *testing.T) {
	...
	for range 8 {
		go func() {
			for range 200 {
				ovector, err := re.Match([]byte("abc 1234"), 0, 0, DefaultLimits())
